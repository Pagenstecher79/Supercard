...
class ScLabelsEditor extends LitElement {
  ...
  _commit(newList) {
    this.dispatchEvent(new CustomEvent('labels-update', { detail: { labels_list: newList } }));
  }
      ...
      <details class="inner-section" ?open=${this._outerOpen} @toggle=${e => this._outerOpen = e.target.open}>
        ...
        <div class="inner-content">
          ${list.map((item, idx) => {
            ...
            if (item.global_id && item.global_id !== 'manual') {
              const foundAlias = (this.slot?.global_entities || []).find(g => g.id === item.global_id);
              if (foundAlias) mainResolvedEntity = foundAlias.entity;
            }
...
Object.assign(window.SupercardModules['labels'], (() => {
  ...
  function update({ hass, config }) {
    ...
    const labelsResolved = list.map((item, idx) => {
      // --- ALIAS RESOLVER (Main) ---
      let resolvedEntity = item.entity || '';
      let resolvedAttribute = item.attribute || null;

      if (item.global_id && item.global_id !== 'manual') {
        const globalEntities = config.global_entities || [];
        const foundAlias = globalEntities.find(g => g.id === item.global_id);
        if (foundAlias) {
          resolvedEntity = foundAlias.entity;
          resolvedAttribute = foundAlias.attribute;
        }
  ...
  function renderCustomBlock(commitFn, hass, slot) {
    return html`<sc-labels-editor .slot=${slot} .hass=${hass}
      @labels-update=${e => commitFn('__merge__', e.detail)}>
    </sc-labels-editor>`;
  }
